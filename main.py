...
def load_config():
    ...
    config = {
        ...
        "LLM_CONFIG": {
            "ENABLED": config_data.get("llm", {}).get("enabled", False),
            "PROVIDER": config_data.get("llm", {}).get("provider", "openai"),
            "API_KEY": os.environ.get("LLM_API_KEY", "").strip() or config_data.get("llm", {}).get("api_key", ""),
            "MODEL": config_data.get("llm", {}).get("model", "gpt-4o-mini"),
            "MAX_TOKENS": config_data.get("llm", {}).get("max_tokens", 500),
...
class LLMContentAnalyzer:
    ...
    def __init__(self, config: Dict):
        self.config = config
        self.enabled = config.get("LLM_CONFIG", {}).get("ENABLED", False)
        self.provider = config.get("LLM_CONFIG", {}).get("PROVIDER", "openai")
        self.api_key = config.get("LLM_CONFIG", {}).get("API_KEY", "")
        self.model = config.get("LLM_CONFIG", {}).get("MODEL", "gpt-4o-mini")
        self.max_tokens = config.get("LLM_CONFIG", {}).get("MAX_TOKENS", 500)
        self.temperature = config.get("LLM_CONFIG", {}).get("TEMPERATURE", 0.7)
        self.timeout = config.get("LLM_CONFIG", {}).get("TIMEOUT", 30)
...
"""
    ...
    def _fallback_analysis(self, title: str, content: str) -> Dict:
        ...
        for tag_name, keywords in creative_patterns:
            if any(k in full_text for k in keywords):
                creative_tags.append(tag_name)

        return {
            "event_type": event_type,
            "creative_points": creative_tags if creative_tags else ["标准活动策划"],
            "innovation_level": "中",
            "target_audience": "大众消费者",
            "reference_value": "需要大模型API才能进行深度分析",
            "execution_tips": "建议查看原文获取详细信息",
            "analysis_source": "规则匹配",
        }
...
def format_analysis_for_wework(analysis: Dict, title: str, url: str, source: str, rank: int) -> str:
    ...
    lines.append(f"📍 来源: {source} | TOP{rank}")
    lines.append(f"🔍 分析来源: {analysis.get('analysis_source', '未知')}")
